...
exports.allReady = allReady;
exports.findLoginAccount = findLoginAccount;
exports.updateLoginPassword = updateLoginPassword;
...
exports.getTenantAlarmCenterData = getTenantAlarmCenterData;
...
exports.replaceAdminState = replaceAdminState;
exports.createTenantWithAdmin = createTenantWithAdmin;
exports.updateTenant = updateTenant;
exports.deleteTenantCascade = deleteTenantCascade;
...
function getPool() {
    ...
    if (!globalThis.__smartFirePgPool) {
        ...
        globalThis.__smartFirePgPool = new pg_1.Pool({
...
async function migratePasswordHashes(client) {
    ...
    for (const row of result.rows) {
        ...
        const passwordHash = (0, password_1.hashPassword)(row.password || "123456");
...
async function seedIfEmpty(client) {
    ...
    ('account-platform-1', 'platform_admin', '', '${(0, password_1.hashPassword)("Admin123456")}', '骞冲彴瓒呯骇绠＄悊鍛?, 'platform', 'platform_super_admin', NULL, false),
    ('account-tenant-hx-1', 'hx_admin', '', '${(0, password_1.hashPassword)("Hx123456")}', '鍗庢槦鍒堕€犵鐞嗗憳', 'tenant', 'tenant_level_1', 'tenant-huaxing', true),
    ('account-tenant-ah-1', 'ah_admin', '', '${(0, password_1.hashPassword)("Ah123456")}', '瀹夊拰鍟嗕笟涓績绠＄悊鍛?, 'tenant', 'tenant_level_1', 'tenant-anhe', true)
    ON CONFLICT (id) DO NOTHING;
...
async function seedSpatialFoundation(client) {
    const existing = await client.query("SELECT COUNT(*)::text AS count FROM tenant_sites");
    if (Number(existing.rows[0]?.count ?? 0) > 0) {
        return;
    }
    await client.query(`
...
async function queryReady(sql, params = []) {
    await ensureDatabase();
    return query(sql, params);
}
async function getReady(sql, params = []) {
    const result = await queryReady(sql, params);
    return result.rows[0];
}
async function allReady(sql, params = []) {
    const result = await queryReady(sql, params);
    return result.rows;
}
...
async function findLoginAccount(username, password) {
    const row = await getReady("SELECT * FROM login_accounts WHERE username = $1 LIMIT 1", [username]);
    if (!row) {
        return null;
    }
    const passwordMatched = (0, password_1.verifyPassword)(password, row.password_hash) || (!!row.password && row.password === password);
    if (!passwordMatched) {
        return null;
    }
    if (!row.password_hash) {
        await queryReady("UPDATE login_accounts SET password_hash = $1 WHERE id = $2", [(0, password_1.hashPassword)(password), row.id]);
    }
    return {
        id: row.id,
        username: row.username,
...
async function updateLoginPassword(username, nextPassword) {
    const passwordHash = (0, password_1.hashPassword)(nextPassword);
    await queryReady("UPDATE login_accounts SET password = '', password_hash = $1, must_change_password = FALSE WHERE username = $2", [passwordHash, username]);
}
async function getTenantById(tenantId) {
    if (!tenantId) {
        return null;
    }
    const row = await getReady("SELECT * FROM tenants WHERE id = $1", [tenantId]);
    return row ? mapTenant(row) : null;
}
async function getPlatformOverview() {
    return {
        tenants: (await allReady("SELECT * FROM tenants ORDER BY created_at ASC")).map(mapTenant),
        plans: (await allReady("SELECT * FROM plans ORDER BY name ASC")).map(mapPlan),
        subscriptions: (await allReady("SELECT * FROM subscriptions ORDER BY start_date DESC")).map(mapSubscription),
...
async function getTenantOverview(tenantId) {
    ...
    const planRow = subscription ? await getReady("SELECT * FROM plans WHERE id = $1 LIMIT 1", [subscription.planId]) : undefined;
    const settingsRow = await getReady("SELECT * FROM notification_settings WHERE tenant_id = $1 LIMIT 1", [tenantId]);
    const quotaRow = await getReady("SELECT * FROM quota_usage WHERE tenant_id = $1 LIMIT 1", [tenantId]);
    return {
        tenant,
        subscription,
        plan: planRow ? mapPlan(planRow) : null,
        devices: (await allReady("SELECT * FROM tenant_devices WHERE tenant_id = $1 ORDER BY name ASC", [tenantId])).map(mapTenantDevice),
        alarms: (await allReady("SELECT * FROM tenant_alarms WHERE tenant_id = $1 ORDER BY time DESC", [tenantId])).map(mapAlarm),
        users: (await allReady("SELECT * FROM tenant_users WHERE tenant_id = $1 ORDER BY username ASC", [tenantId])).map(mapTenantUser),
...
async function getTenantSpatialModel(tenantId) {
    const [sites, buildings, floors, drawings, gateways, devicePoints, statusSnapshots, recentEvents, devices] = await Promise.all([
        ...
        allReady("SELECT * FROM tenant_buildings WHERE tenant_id = $1 ORDER BY name ASC", [tenantId]),
        allReady("SELECT * FROM tenant_floors WHERE tenant_id = $1 ORDER BY level_index ASC", [tenantId]),
        allReady("SELECT * FROM tenant_drawings WHERE tenant_id = $1 ORDER BY updated_at DESC", [tenantId]),
        allReady("SELECT * FROM tenant_gateways WHERE tenant_id = $1 ORDER BY name ASC", [tenantId]),
        allReady("SELECT * FROM tenant_device_points WHERE tenant_id = $1 ORDER BY updated_at DESC", [tenantId]),
        allReady("SELECT * FROM device_status_snapshots WHERE tenant_id = $1 ORDER BY updated_at DESC", [tenantId]),
        allReady("SELECT * FROM raw_device_events WHERE tenant_id = $1 ORDER BY reported_at DESC LIMIT 12", [tenantId]),
        allReady("SELECT id FROM tenant_devices WHERE tenant_id = $1", [tenantId]),
    ]);
...
async function getTenantHistoryData(tenantId) {
    const [alarms, rawEvents, devices] = await Promise.all([
        allReady("SELECT * FROM tenant_alarms WHERE tenant_id = $1 ORDER BY time DESC LIMIT 800", [tenantId]),
        allReady("SELECT * FROM raw_device_events WHERE tenant_id = $1 ORDER BY reported_at DESC LIMIT 1500", [tenantId]),
        allReady("SELECT * FROM tenant_devices WHERE tenant_id = $1 ORDER BY name ASC", [tenantId]),
    ]);
    return {
...
async function insertAuditLog(client, input) {
    ...
    await client.query(`INSERT INTO audit_logs (id, tenant_id, actor_scope, actor_name, actor_role, action, target_type, target_id, result, detail, created_at)
...
async function insertDutyLog(client, input) {
    const createdAt = input.createdAt ?? formatLocalTimestamp();
    await client.query(`INSERT INTO duty_logs (id, tenant_id, schedule_id, log_type, content, operator_name, created_at)
     VALUES ($1,$2,$3,$4,$5,$6,$7)`, [createId("duty-log"), input.tenantId, input.scheduleId ?? null, input.logType, input.content, input.operatorName, createdAt]);
}
async function insertAlarmTimeline(client, input) {
    const createdAt = input.createdAt ?? formatLocalTimestamp();
    await client.query(`INSERT INTO alarm_logs (id, tenant_id, alarm_id, action, from_status, to_status, operator_name, operator_role, note, attachments, created_at)
...
async function createNotificationRecordsForAlarm(client, tenantId, alarmId, level, content, createdAt) {
    const templates = await client.query("SELECT * FROM notification_templates WHERE tenant_id = $1 AND level = $2 AND enabled = TRUE ORDER BY created_at ASC", [tenantId, level]);
    for (const template of templates.rows) {
        await client.query(`INSERT INTO notification_records (id, tenant_id, alarm_id, template_id, channel, level, target_name, content, status, retry_count, last_error, created_at, updated_at)
...
async function getTenantAlarmCenterData(tenantId) {
    await ensureDatabase();
    const alarms = await allReady("SELECT * FROM tenant_alarms WHERE tenant_id = $1 ORDER BY time DESC, id DESC LIMIT 400", [tenantId]);
    if (alarms.length === 0) {
        return [];
    }
    const timelines = await allReady("SELECT * FROM alarm_logs WHERE tenant_id = $1 ORDER BY created_at DESC", [tenantId]);
...
async function updateTenantAlarmWorkflow(tenantId, input) {
    ...
    return withClient(async (client) => {
        ...
        try {
            ...
            const nextProcessStatus = input.nextStatus === "处理中"
                ? "处理中"
...
async function getTenantNotificationCenterData(tenantId) {
    ...
    const [templates, records] = await Promise.all([
        allReady("SELECT * FROM notification_templates WHERE tenant_id = $1 ORDER BY updated_at DESC", [tenantId]),
        allReady("SELECT * FROM notification_records WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 300", [tenantId]),
    ]);
    return {
...
async function retryNotificationRecord(tenantId, recordId) {
    await ensureDatabase();
    const now = formatLocalTimestamp();
    await queryReady(`UPDATE notification_records
     SET status = 'sent',
...
async function getTenantAuditLogs(tenantId) {
    await ensureDatabase();
    const rows = await allReady("SELECT * FROM audit_logs WHERE tenant_id = $1 OR tenant_id IS NULL ORDER BY created_at DESC LIMIT 500", [tenantId]);
...
async function getTenantSystemHealth(tenantId) {
    ...
    const [devices, events, audits] = await Promise.all([
        allReady("SELECT * FROM tenant_devices WHERE tenant_id = $1", [tenantId]),
        allReady("SELECT * FROM raw_device_events WHERE tenant_id = $1 ORDER BY reported_at DESC LIMIT 20", [tenantId]),
        allReady("SELECT * FROM audit_logs WHERE tenant_id = $1 AND result = 'error' ORDER BY created_at DESC LIMIT 20", [tenantId]),
    ]);
...
async function getDutyCenterData(tenantId) {
    ...
    const [shiftRows, scheduleRows, logRows, openAlarmRow] = await Promise.all([
        allReady("SELECT * FROM duty_shifts WHERE tenant_id = $1 ORDER BY start_time ASC", [tenantId]),
        allReady("SELECT * FROM duty_schedules WHERE tenant_id = $1 ORDER BY duty_date DESC, shift_id ASC LIMIT 30", [tenantId]),
        allReady("SELECT * FROM duty_logs WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 200", [tenantId]),
        getReady("SELECT COUNT(*)::int AS count FROM tenant_alarms WHERE tenant_id = $1 AND process_status <> '已处理'", [tenantId]),
    ]);
...
async function createDutySchedule(tenantId, input) {
    ...
    await withClient(async (client) => {
       ...
       VALUES ($1,$2,$3,$4,$5,$6,$7,'scheduled',NULL,NULL,'')`, [id, tenantId, input.dutyDate, input.shiftId, input.assigneeName, input.assigneePhone, input.assignedBy]);
        await insertDutyLog(client, {
            tenantId,
            scheduleId: id,
...
async function handoverDutySchedule(tenantId, input) {
    ...
    await withClient(async (client) => {
        await client.query("BEGIN");
        try {
            const openAlarmRow = await client.query("SELECT COUNT(*)::int AS count FROM tenant_alarms WHERE tenant_id = $1 AND process_status <> '已处理'", [tenantId]);
         ...
         WHERE tenant_id = $3 AND id = $4`, [createdAt, input.note, tenantId, input.scheduleId]);
            if (input.nextScheduleId) {
                await client.query(`UPDATE duty_schedules
           SET status = 'active', started_at = COALESCE(started_at, $1)
           WHERE tenant_id = $2 AND id = $3`, [createdAt, tenantId, input.nextScheduleId]);
            }
            await insertDutyLog(client, {
                tenantId,
...
async function getInspectionCenterData(tenantId) {
    ...
    const [tasks, records, issues, maintenanceRecords] = await Promise.all([
        allReady("SELECT * FROM inspection_tasks WHERE tenant_id = $1 ORDER BY due_date DESC, created_at DESC LIMIT 200", [tenantId]),
        allReady("SELECT * FROM inspection_records WHERE tenant_id = $1 ORDER BY inspected_at DESC LIMIT 300", [tenantId]),
        allReady("SELECT * FROM issues WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 200", [tenantId]),
        allReady("SELECT * FROM maintenance_records WHERE tenant_id = $1 ORDER BY next_due_date ASC LIMIT 200", [tenantId]),
    ]);
    return {
        generatedAt: formatLocalTimestamp(),
    ...
    await withClient(async (client) => {
        ...
        try {
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7)`, [recordId, tenantId, input.taskId, input.result, input.note, input.inspectedBy, createdAt]);
           ...
           VALUES ($1,$2,'inspection',$3,$4,'medium','未整改',$5,$6,NULL,NULL,$7)`, [issueId, tenantId, input.taskId, `${task.title} 发现异常`, input.note, formatLocalDate(new Date(Date.now() + 3 * 24 * 60 * 60 * 1000)), createdAt]);
            }
            await insertAuditLog(client, {
                tenantId,
                actorScope: "tenant",
...
async function getAdminState() {
    return {
        ...
        subscriptions: (await allReady("SELECT * FROM subscriptions ORDER BY start_date DESC")).map(mapSubscription),
        platformUsers: (await allReady("SELECT * FROM platform_users ORDER BY username ASC")).map(mapPlatformUser),
        tenantUsers: (await allReady("SELECT * FROM tenant_users ORDER BY tenant_id ASC, username ASC")).map(mapTenantUser),
        tenantDevices: (await allReady("SELECT * FROM tenant_devices ORDER BY tenant_id ASC, name ASC")).map(mapTenantDevice),
        tenantAlarms: (await allReady("SELECT * FROM tenant_alarms ORDER BY time DESC")).map(mapAlarm),
        notificationSettings: (await allReady("SELECT * FROM notification_settings ORDER BY tenant_id ASC")).map(mapNotification),
        quotaUsage: (await allReady("SELECT * FROM quota_usage ORDER BY tenant_id ASC")).map(mapQuota),
    };
}
async function replaceAdminState(input) {
    await ensureDatabase();
    await withClient(async (client) => {
        await client.query("BEGIN");
        try {
      ...
      `);
            ...
            for (const item of input.plans ?? []) {
                await client.query("INSERT INTO plans (id, name, code, status, price_monthly, max_devices, max_users, sms_quota, feature_keys, description) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10)", [
                    item.id,
                    item.name,
                    item.code,
                    item.status,
            ...
            for (const item of input.tenantUsers ?? []) {
                await client.query("INSERT INTO tenant_users (id, tenant_id, username, phone, role_key, status, sms_enabled, message_types, note) VALUES ($1,$2,$3,$4,$5,$6,$7,$8::jsonb,$9)", [
                    item.id,
                    item.tenantId,
                    item.username,
                    item.phone,
            ...
            for (const item of input.tenantDevices ?? []) {
                await client.query("INSERT INTO tenant_devices (id, tenant_id, name, type, area, installation_location, status, last_report_at, notes) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)", [
                    item.id,
                    item.tenantId,
                    item.name,
                    item.type,
            ...
            for (const item of input.notificationSettings ?? []) {
                await client.query("INSERT INTO notification_settings (id, tenant_id, alarm_threshold, notification_enabled, map_placeholder, remark) VALUES ($1,$2,$3,$4,$5,$6)", [item.id, item.tenantId, item.alarmThreshold, !!item.notificationEnabled, item.mapPlaceholder, item.remark]);
            }
            for (const item of input.quotaUsage ?? []) {
                await client.query("INSERT INTO quota_usage (id, tenant_id, device_count, user_count, sms_used) VALUES ($1,$2,$3,$4,$5)", [item.id, item.tenantId, item.deviceCount, item.userCount, item.smsUsed]);
            }
            await client.query("COMMIT");
        }
        catch (error) {
...
async function createTenantWithAdmin(input) {
    ...
    const passwordHash = (0, password_1.hashPassword)(input.admin.password);
    await withClient(async (client) => {
        await client.query("BEGIN");
        try {
            await client.query("INSERT INTO tenants (id, name, code, industry, contact_name, contact_phone, status, created_at, note) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)", [
                tenantId,
                input.tenant.name,
                input.tenant.code,
            ...
            await client.query("INSERT INTO login_accounts (id, username, password, password_hash, display_name, scope, role_key, tenant_id, must_change_password) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)", [
                loginAccountId,
                input.admin.username,
                "",
                passwordHash,
...
async function updateTenant(input) {
    await queryReady("UPDATE tenants SET name = $1, code = $2, industry = $3, contact_name = $4, contact_phone = $5, status = $6, note = $7 WHERE id = $8", [input.name, input.code, input.industry, input.contactName, input.contactPhone, input.status, input.note, input.id]);
    const row = await getReady("SELECT * FROM tenants WHERE id = $1", [input.id]);
    return row ? mapTenant(row) : null;
}
...
async function listTenantUsers(tenantId) {
    return (await allReady("SELECT * FROM tenant_users WHERE tenant_id = $1 ORDER BY username ASC", [tenantId])).map(mapTenantUser);
}
async function upsertTenantUser(tenantId, input) {
    await ensureDatabase();
    ...
    const row = await getReady("SELECT * FROM tenant_users WHERE id = $1", [id]);
    return mapTenantUser(row);
}
async function deleteTenantUser(tenantId, id) {
...
async function upsertTenantDevicePoint(tenantId, input) {
    ...
    const existing = input.id
        ? await getReady("SELECT * FROM tenant_device_points WHERE id = $1 AND tenant_id = $2", [input.id, tenantId])
        : await getReady("SELECT * FROM tenant_device_points WHERE device_id = $1 AND tenant_id = $2", [input.deviceId, tenantId]);
...
async function upsertTenantDevice(tenantId, input) {
    await ensureDatabase();
    ...
    const row = await getReady("SELECT * FROM tenant_devices WHERE id = $1 AND tenant_id = $2", [id, tenantId]);
    if (!row) {
        throw new Error("设备保存成功后未能读取到最新记录");
    }
    return mapTenantDevice(row);
}
async function deleteTenantDevice(tenantId, id) {
...
async function syncDeviceRuntimeFromAlarms(client, tenantId, deviceId, reportedAt, gatewayId) {
    ...
    await client.query("UPDATE tenant_device_points SET status_style = $1, updated_at = $2 WHERE tenant_id = $3 AND device_id = $4", [mapped.pointStatusStyle, reportedAt, tenantId, deviceId]);
    return mapped;
}
...
async function updateTenantAlarmProcessStatus(tenantId, alarmId, processStatus) {
    ...
    await withClient(async (client) => {
        await client.query("BEGIN");
        try {
            const alarm = await client.query("SELECT * FROM tenant_alarms WHERE tenant_id = $1 AND id = $2 LIMIT 1", [tenantId, alarmId]);
            if (alarm.rowCount === 0) {
                throw new Error("ALARM_NOT_FOUND");
            }
            ...
            await client.query("UPDATE tenant_alarms SET workflow_status = $1, last_operator_name = $2, completed_at = CASE WHEN $1 = '已完成' THEN $3 ELSE completed_at END WHERE tenant_id = $4 AND id = $5", [processStatus === "已处理" ? "已完成" : processStatus, "企业控制台", updatedAt, tenantId, alarmId]);
            await insertAlarmTimeline(client, {
                tenantId,
                alarmId,
                action: "快捷处理",
                fromStatus: normalizeWorkflowStatus(alarmRow.workflow_status ?? alarmRow.process_status),
                toStatus: processStatus === "已处理" ? "已完成" : processStatus,
                operatorName: "企业控制台",
            ...
            const refreshed = await client.query("SELECT * FROM tenant_alarms WHERE tenant_id = $1 AND id = $2 LIMIT 1", [tenantId, alarmId]);
...
async function ingestDeviceEvent(input) {
    await ensureDatabase();
    const device = await getReady("SELECT * FROM tenant_devices WHERE tenant_id = $1 AND id = $2 LIMIT 1", [
        input.tenantId,
        input.deviceId,
    ]);
    if (!device) {
    ...
    const pointStatusStyle = nextStatus === "alarm"
        ? "alarm"
        : nextStatus === "fault"
            ? "fault"
            : nextStatus === "offline"
    ...
    await withClient(async (client) => {
        ...
        `, [
            ...
            await client.query("UPDATE tenant_device_points SET status_style = $1, updated_at = $2 WHERE tenant_id = $3 AND device_id = $4", [pointStatusStyle, reportedAt, input.tenantId, input.deviceId]);
            ...
            `, [
                ...
                await createNotificationRecordsForAlarm(client, input.tenantId, alarmId, nextStatus === "fault" ? "fault" : "alarm", `${device.name} / ${alarmTypeLabel} / ${reportedAt}`, reportedAt);
                await client.query(`INSERT INTO system_health_snapshots (id, tenant_id, metric_code, metric_name, metric_value, metric_unit, level, detail, recorded_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`, [
    ...
    (0, server_1.publishTenantEvent)(input.tenantId, {
