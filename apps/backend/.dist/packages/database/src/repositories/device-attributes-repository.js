...
exports.coreDeviceAttributeDefinitions = void 0;
exports.ensureCoreDeviceAttributeDefinitions = ensureCoreDeviceAttributeDefinitions;
exports.listTenantDeviceAttributes = listTenantDeviceAttributes;
exports.listTenantDeviceAttributesWithDefaults = listTenantDeviceAttributesWithDefaults;
exports.upsertTenantDeviceAttribute = upsertTenantDeviceAttribute;
exports.disableTenantDeviceAttribute = disableTenantDeviceAttribute;
const client_1 = require("../client");
const errors_1 = require("../errors");
const _shared_1 = require("./_shared");
const attributeTypes = new Set(["auto", "text", "number", "date", "boolean"]);
exports.coreDeviceAttributeDefinitions = [
...
function mapDeviceAttribute(row) {
    return {
...
async function ensureCoreDeviceAttributeDefinitions(executor, tenantId) {
    (0, errors_1.assertTenantId)(tenantId);
    const now = (0, _shared_1.formatLocalTimestamp)();
    for (const definition of exports.coreDeviceAttributeDefinitions) {
...
async function listTenantDeviceAttributes(tenantId) {
    (0, errors_1.assertTenantId)(tenantId);
    return (0, client_1.withDbClient)(async (client) => listTenantDeviceAttributesWithDefaults(client, tenantId));
}
async function listTenantDeviceAttributesWithDefaults(executor, tenantId) {
...
async function assertLabelAvailable(executor, tenantId, label, fieldKey) {
    const result = await executor.query("SELECT field_key FROM tenant_device_attribute_definitions WHERE tenant_id = $1 AND label = $2 AND field_key <> $3 AND enabled = TRUE LIMIT 1", [tenantId, label, fieldKey]);
    if (result.rows[0]) {
        throw new Error("字段名称已存在，请换一个名称");
    }
}
async function customAttributeHasValues(executor, tenantId, fieldKey) {
...
function canChangeTypeWithExistingValues(fromType, toType) {
    if (fromType === toType)
        return true;
    if (toType === "auto")
        return true;
    if (fromType === "auto" && toType === "text")
        return true;
    return false;
}
async function upsertTenantDeviceAttribute(executor, tenantId, input) {
    (0, errors_1.assertTenantId)(tenantId);
    await ensureCoreDeviceAttributeDefinitions(executor, tenantId);
    ...
    if (existing?.isCore) {
      ...
      `, [
        ...
        const fieldType = (input.fieldType || "auto");
...
async function disableTenantDeviceAttribute(executor, tenantId, fieldKey) {
    (0, errors_1.assertTenantId)(tenantId);
    await ensureCoreDeviceAttributeDefinitions(executor, tenantId);
    ...
    await executor.query("UPDATE tenant_device_attribute_definitions SET enabled = FALSE, updated_at = $1 WHERE tenant_id = $2 AND field_key = $3", [(0, _shared_1.formatLocalTimestamp)(), tenantId, fieldKey]);
    return { success: true };
}
