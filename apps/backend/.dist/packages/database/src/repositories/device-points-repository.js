...
async function listTenantDevicePoints(tenantId) {
    ...
    const rows = await (0, client_1.queryDb)("SELECT * FROM tenant_device_points WHERE tenant_id = $1 ORDER BY updated_at DESC", [tenantId]);
...
async function upsertTenantDevicePoint(tenantId, input) {
    ...
    return (0, transaction_1.withTransaction)(async (client) => {
        ...
        const drawing = await client.query("SELECT id, floor_id FROM tenant_drawings WHERE tenant_id = $1 AND id = $2 LIMIT 1", [tenantId, input.drawingId]);
