...
exports.findTenantDeviceIdByCode = findTenantDeviceIdByCode;
exports.previewTenantDeviceLifecycle = previewTenantDeviceLifecycle;
exports.updateTenantDeviceLifecycle = updateTenantDeviceLifecycle;
...
async function countByDevice(executor, sql, params) {
    const result = await executor.query(sql, params);
    return new Map(result.rows.map((row) => [String(row.device_id), Number(row.count ?? 0)]));
}
async function listTenantDevices(tenantId, lifecycle) {
    (0, errors_1.assertTenantId)(tenantId);
    const filter = normalizeLifecycleFilter(lifecycle);
    const result = filter === "all"
        ? await (0, client_1.queryDb)("SELECT * FROM tenant_devices WHERE tenant_id = $1 ORDER BY name ASC", [tenantId])
        : await (0, client_1.queryDb)("SELECT * FROM tenant_devices WHERE tenant_id = $1 AND COALESCE(lifecycle_status, 'active') = $2 ORDER BY name ASC", [tenantId, filter]);
    return result.rows.map(_shared_1.mapTenantDevice);
}
async function getTenantDeviceById(executor, tenantId, id) {
    (0, errors_1.assertTenantId)(tenantId);
    const result = await executor.query(`SELECT id, tenant_id, name, area, installation_location, gateway_id, lifecycle_status, disabled_at, disabled_reason
     FROM tenant_devices
...
async function upsertTenantDevice(executor, tenantId, input) {
    (0, errors_1.assertTenantId)(tenantId);
    await (0, device_attributes_repository_1.ensureCoreDeviceAttributeDefinitions)(executor, tenantId);
    const deviceCode = String(input.deviceCode ?? "").trim();
    const id = input.id ?? (deviceCode ? await findTenantDeviceIdByCode(executor, tenantId, deviceCode) : null) ?? (0, _shared_1.createId)("device");
    ...
    const customAttributes = input.customAttributes && typeof input.customAttributes === "object" && !Array.isArray(input.customAttributes)
...
async function findTenantDeviceIdByCode(executor, tenantId, deviceCode) {
    (0, errors_1.assertTenantId)(tenantId);
...
async function previewTenantDeviceLifecycle(executor, tenantId, input) {
    (0, errors_1.assertTenantId)(tenantId);
    ...
    const pointCounts = await countByDevice(executor, "SELECT device_id, COUNT(*)::int AS count FROM tenant_device_points WHERE tenant_id = $1 AND device_id = ANY($2::text[]) GROUP BY device_id", [tenantId, deviceIds]);
    const openAlarmCounts = await countByDevice(executor, `
      ...
      GROUP BY device_id
    `, [tenantId, deviceIds]);
    const rawEventCounts = await countByDevice(executor, "SELECT device_id, COUNT(*)::int AS count FROM raw_device_events WHERE tenant_id = $1 AND device_id = ANY($2::text[]) GROUP BY device_id", [tenantId, deviceIds]);
    const maintenanceCounts = await countByDevice(executor, "SELECT device_id, COUNT(*)::int AS count FROM maintenance_records WHERE tenant_id = $1 AND device_id = ANY($2::text[]) GROUP BY device_id", [tenantId, deviceIds]);
...
async function updateTenantDeviceLifecycle(executor, tenantId, input) {
    (0, errors_1.assertTenantId)(tenantId);
    const action = normalizeLifecycleAction(input.action);
    ...
    return {
        ...
        results: [
            ...preview.devices.map((item) => ({
                ...
                status: updatedIds.has(item.deviceId) ? "updated" : "skipped",
            ...
            ...normalizeDeviceIds(input.deviceIds)
                ...
                .map((deviceId) => ({ deviceId, status: "failed", message: "设备不存在" })),
        ],
    };
}
