...
import type {
  DeviceDuplicatePolicy,
  DeviceImportError,
  DeviceImportPreview,
  DeviceImportRow,
  TenantDeviceAttributeDefinition,
} from "../../../../types/saas";
import { formatLocalTimestamp } from "./_shared";
import { listTenantDeviceAttributesWithDefaults } from "./device-attributes-repository";
import { upsertTenantDevice } from "./devices-repository";
...
function getDefinitionByKey(definitions: TenantDeviceAttributeDefinition[], fieldKey: string) {
  return definitions.find((definition) => definition.fieldKey === fieldKey);
}

function buildLabelMap(definitions: TenantDeviceAttributeDefinition[]) {
  return new Map(definitions.filter((item) => item.enabled).map((item) => [item.label, item]));
}

async function loadExistingDeviceCodes(executor: DbExecutor, tenantId: string, codes: string[]) {
  if (codes.length === 0) return new Map<string, string>();
  const result = await executor.query(
    "SELECT device_code, id FROM tenant_devices WHERE tenant_id = $1 AND device_code = ANY($2::text[])",
    [tenantId, codes],
  );
  return new Map(result.rows.map((row) => [String(row.device_code), String(row.id)]));
}

async function loadExistingCustomAttributes(executor: DbExecutor, tenantId: string, ids: string[]) {
  if (ids.length === 0) return new Map<string, Record<string, string | number | boolean | null>>();
...
export async function previewTenantDeviceImport(
  executor: DbExecutor,
  tenantId: string,
  input: { fileName?: string; headers?: string[]; rows?: DeviceImportRow[] },
): Promise<DeviceImportPreview> {
  assertTenantId(tenantId);
  const definitions = await listTenantDeviceAttributesWithDefaults(executor, tenantId);
...
function readCoreValue(definitions: TenantDeviceAttributeDefinition[], row: DeviceImportRow, fieldKey: string) {
  const definition = getDefinitionByKey(definitions, fieldKey);
  return definition ? normalizeCell(row.values[definition.label]) : "";
}

export async function commitTenantDeviceImport(
  executor: DbExecutor,
  tenantId: string,
  input: {
    fileName?: string;
    ...
    duplicatePolicy?: DeviceDuplicatePolicy;
  },
) {
  assertTenantId(tenantId);
  ...
  const existingCodes = await loadExistingDeviceCodes(
    executor,
    tenantId,
    rows.map((row) => normalizeCell(row.values[codeLabel])).filter(Boolean),
  );
  ...
  for (const row of rows) {
    ...
    const customAttributes: Record<string, string | number | boolean | null> = existingId
      ? { ...(existingCustomAttributes.get(existingId) ?? {}) }
      : {};
    for (const definition of enabledDefinitions) {
      if (definition.isCore) continue;
      if (!Object.prototype.hasOwnProperty.call(row.values, definition.label)) continue;
    ...
    const device = await upsertTenantDevice(executor, tenantId, {
      id: existingId,
      deviceCode,
      name,
      ...
      lastReportAt: readCoreValue(enabledDefinitions, row, "lastReportAt") || formatLocalTimestamp(),
