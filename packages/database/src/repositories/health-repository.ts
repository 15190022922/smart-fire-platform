...
import { buildSystemHealthPayload, buildSystemHealthMetric, formatLocalTimestamp, mapAuditLog, normalizeStatusText, parseDbDate } from "./_shared";

export async function getTenantSystemHealth(tenantId: string) {
  assertTenantId(tenantId);
  const now = new Date();
  const [devices, events, audits] = await Promise.all([
    queryDb("SELECT * FROM tenant_devices WHERE tenant_id = $1 AND COALESCE(lifecycle_status, 'active') = 'active'", [tenantId]),
    queryDb("SELECT * FROM raw_device_events WHERE tenant_id = $1 ORDER BY reported_at DESC LIMIT 20", [tenantId]),
    queryDb("SELECT * FROM audit_logs WHERE tenant_id = $1 AND result = 'error' ORDER BY created_at DESC LIMIT 20", [tenantId]),
  ]);

  const totalDevices = devices.rows.length || 1;
  const onlineDevices = devices.rows.filter((row: any) => normalizeStatusText(row.status) !== "离线").length;
  const onlineRate = (onlineDevices / totalDevices) * 100;

  const latestAlarmEvent = events.rows.find((row: any) => String(row.event_type) === "alarm") ?? events.rows[0];
  ...
  return buildSystemHealthPayload({
    generatedAt: formatLocalTimestamp(),
    metrics: [
      buildSystemHealthMetric({
        code: "realtime_link",
        name: "实时连接状态",
    ...
    recentErrors: audits.rows.map(mapAuditLog),
    latestEvents: events.rows.map((row: any) => ({
      id: row.id,
