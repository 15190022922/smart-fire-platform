      ...
      <SectionCard title="功能目录" description="功能可见性由套餐能力与角色权限共同决定。">
        <div className="space-y-3">
          {features.map((feature) => (
            <div key={feature.key} className="rounded-2xl border border-[color:var(--border)] bg-[var(--surface)] px-4 py-4 shadow-[var(--panel-shadow)]">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {plans.filter((plan) => plan.featureKeys.includes(feature.key)).map((plan) => (
                  <span key={plan.id} className="rounded-full border border-sky-200 bg-sky-50 px-3 py-1 text-xs text-sky-700">
                    {plan.name}
                  </span>
                ))}
              </div>
