    ...
    <FeatureGuard title="可视化主页面" featureKey="dashboard" permissionKey="tenant.dashboard.view">
      <div className="space-y-6">
        <PageHeader
          ...
          aside={
            <div className="rounded-[24px] border border-[color:var(--border)] bg-[var(--surface-muted)] px-4 py-3">
        ...
        <div className="grid gap-4 xl:grid-cols-4">
          {[
            ["当前报警数", alarmCount, "border-rose-200 bg-rose-50 text-rose-700"],
            ["未处理报警", pendingCount, "border-amber-200 bg-amber-50 text-amber-700"],
            ["在线设备", onlineCount, "border-emerald-200 bg-emerald-50 text-emerald-700"],
            ["故障设备", faultCount, "border-cyan-200 bg-cyan-50 text-cyan-700"],
          ].map(([label, value, className]) => (
            <div key={label} className={`rounded-[24px] border p-5 shadow-[var(--panel-shadow)] ${className}`}>
                ...
                return (
                  <div key={label}>
                    ...
                    <div className="h-2.5 rounded-full bg-[var(--surface-muted)]">
                      <div className="h-2.5 rounded-full bg-sky-500" style={{ width: `${ratio}%` }} />
                    </div>
                  </div>
                );
              })}
