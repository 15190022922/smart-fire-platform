...
import { AlarmPoint, FloorZone } from "@/types/platform";
...
export function InteractiveMapPanel({ zones }: { zones: FloorZone[] }) {
  ...
  return (
    <section className="grid min-h-0 grid-rows-[auto_auto_minmax(0,1fr)] rounded-[22px] border border-[color:var(--border-strong)] bg-[var(--surface)] shadow-[var(--panel-shadow)]">
      <div className="flex min-h-0 items-center justify-between border-b border-[color:var(--border)] px-4 py-2.5">
      ...
      <div className="flex min-h-0 items-center justify-between gap-3 border-b border-[color:var(--border)] px-4 py-2">
        ...
        <div className="flex items-center gap-1.5">
          <button
            type="button"
            onClick={() => setScale((current) => clamp(Number((current - 0.1).toFixed(2)), MIN_SCALE, MAX_SCALE))}
            className="rounded-full border border-[color:var(--border)] bg-[var(--surface-strong)] px-3 py-1.5 text-xs text-[color:var(--text-secondary)] transition hover:bg-[var(--surface-muted)]"
          >
            缩小
          </button>
          <button
      ...
      >
            ...
            zone.points.map((point) => {
              ...
              return (
                <button
                  ...
                  className={`absolute h-5 w-5 -translate-x-1/2 -translate-y-1/2 cursor-pointer rounded-full border-2 shadow-sm ${pointTone[point.status]}`}
        ...
        {selectedPoint ? (
          <div className="absolute bottom-4 left-4 z-20 w-[320px] rounded-[18px] border border-[color:var(--border-strong)] bg-[var(--surface-strong)] p-4 shadow-[var(--panel-shadow)]">
            ...
            <dl className="mt-3 space-y-2 text-xs">
              <div className="flex justify-between gap-3">
                ...
                <dd className="max-w-[170px] text-right text-[color:var(--text-primary)]">
                  {selectedPoint.positionLabel}
                </dd>
              </div>
              <div className="flex justify-between gap-3">
        ...
        {hoveredPoint ? (
          <div
            className="pointer-events-none absolute z-20 w-[280px] rounded-[18px] border border-[color:var(--border-strong)] bg-[var(--surface-strong)] p-4 text-left shadow-[var(--panel-shadow)]"
            style={{ left: hoveredPoint.x, top: hoveredPoint.y }}
          >
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-[color:var(--text-primary)]">
                  {hoveredPoint.point.deviceName}
                </p>
                <p className="mt-1 truncate text-xs text-[color:var(--text-muted)]">
                  {hoveredPoint.point.deviceType}
                </p>
              </div>
            ...
            <dl className="mt-3 space-y-2 text-xs">
              <div className="flex justify-between gap-3">
                <dt className="text-[color:var(--text-muted)]">所在位置</dt>
                <dd className="max-w-[150px] text-right text-[color:var(--text-primary)]">
                  {hoveredPoint.point.positionLabel}
                </dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-[color:var(--text-muted)]">当前状态</dt>
              ...
              <div className="flex justify-between gap-3">
                ...
                <dd className="text-right text-[color:var(--text-primary)]">
                  {hoveredPoint.point.lastReportAt}
                </dd>
              </div>
            </dl>
          </div>
